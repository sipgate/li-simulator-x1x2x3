...
RUN --mount=type=cache,target=/root/.m2 mvn -B -P spring-boot-application -s maven_settings.xml de.qaware.maven:go-offline-maven-plugin:resolve-dependencies

# compile the jar
COPY src /usr/src/build/src
ARG MVN_ADDITIONAL_ARGS
RUN --mount=type=cache,target=/root/.m2 mvn -B -P spring-boot-application -s maven_settings.xml clean package $MVN_ADDITIONAL_ARGS
