...
RUN mvn -B -s maven_settings.xml de.qaware.maven:go-offline-maven-plugin:resolve-dependencies

# compile the jar
COPY src /usr/src/build/src
ARG MVN_ADDITIONAL_ARGS
RUN mvn -B -s maven_settings.xml clean package $MVN_ADDITIONAL_ARGS
