...
public class PingController {
    ...
    @PostMapping("/ping")
    public ResponseEntity<Map<String, String>> ping() throws JAXBException, IOException, InterruptedException, InvocationTargetException, NoSuchMethodException, InstantiationException, IllegalAccessException {
        ...
        if (resp instanceof PingResponse p) {
            return ResponseEntity.ok(Map.of("ok", p.getOK().value()));
        }

        return ResponseEntity.internalServerError().body(Map.of("error", "PingRequest did not respond with PingResponse, received " + resp.getClass().getSimpleName()));
    }
